...
export function initWs(httpServer, database) {
        ...
        if (data.type === 'msg') {
          ...
          const mediaType = ['image', 'video', 'audio', 'file'].includes(data.media_type) ? data.media_type : 'text'
           if (!to || to === userId || (!text && !media) || text.length > 1000) return
            if (media && (!isStoredUploadPath(media, 'chat') || !(await database.one('SELECT path FROM chat_uploads WHERE path = ? AND user_id = ?', [media, userId])))) return
           const mediaSize = Number(data.media_size)
           const mediaName = typeof data.media_name === 'string' ? data.media_name.slice(0, 255) : null
            if (!await database.one('SELECT id FROM users WHERE id = ?', [to])) return
            const convId = await getOrCreateConversation(database, userId, to)
            const msg = await insertMessage(database, convId, userId, text, media, mediaType, mediaName, Number.isSafeInteger(mediaSize) && mediaSize >= 0 ? mediaSize : null)
            const sender = await database.one('SELECT name, avatar FROM users WHERE id = ?', [userId])
          ...
          const convId = String(data.conversation_id || '')
           const conv = await database.one('SELECT * FROM conversations WHERE id = ?', [convId])
          if (conv && (conv.user1_id === userId || conv.user2_id === userId)) {
             await markRead(database, convId, userId)
